...
export function Navbar() {
  ...
  const localePrimary = t.localeName
  const localeSecondary = "EN"
  ...
  return (
    ...
    >
      ...
      <div className="border-b border-amp-gray-90 px-6 lg:px-16 max-w-[1400px] mx-auto flex items-center gap-2 h-9">
        <span className="text-[11px] text-amp-gray-60 font-semibold uppercase tracking-widest mr-2">
          Region
        </span>
        {regions.map((r) => (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1 rounded text-xs font-semibold transition-all duration-200 ${
              region === r.id
                ? "bg-amp-blue text-white"
                : "text-amp-gray-50 hover:text-white hover:bg-amp-gray-80"
            }`}
          >
            <span>{r.flag}</span>
            <span>{r.label}</span>
