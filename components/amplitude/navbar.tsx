...
const nativeLabels: Record<Region, string> = { pl: "PL", ru: "RU", ar: "AR" }
const navLinks = [
  { key: "products", href: "https://amplitude.com/product" },
  { key: "solutions", href: "https://amplitude.com/solutions" },
  { key: "customers", href: "https://amplitude.com/customers" },
  { key: "analytics", href: "https://amplitude.com/analytics" },
  { key: "pricing", href: "https://amplitude.com/pricing" },
]
export function Navbar() {
  const { locale, setLocale, t, region, setRegion } = useI18n()
  const nativeLabel = nativeLabels[region]
  const navLabels = [t.nav.products, t.nav.solutions, t.nav.customers, t.nav.analytics, t.nav.pricing]
  return (
    <motion.nav initial={{y:-100}} animate={{y:0}} transition={{duration:0.5,ease:"easeOut"}} className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-amp-gray-20 shadow-sm" dir="ltr">
      ...
      <div className="flex items-center justify-between h-16 px-6 lg:px-16 max-w-[1400px] mx-auto">
        <a href="https://amplitude.com" target="_blank" rel="noopener noreferrer"><AmplitudeLogo className="h-7" variant="dark" /></a>
        <ul className="hidden lg:flex items-center gap-8">
          {navLinks.map((link,i)=>(<li key={link.key}><a href={link.href} target="_blank" rel="noopener noreferrer" className="text-amp-gray-70 text-sm font-medium hover:text-amp-gray-100 transition-colors duration-200">{navLabels[i]}</a></li>))}
        </ul>
        <div className="flex items-center gap-3">
          ...
          <a href="https://app.amplitude.com/login" target="_blank" rel="noopener noreferrer" className="hidden sm:inline-block px-5 py-2.5 text-amp-gray-70 text-sm font-semibold border border-amp-gray-20 rounded-md hover:border-amp-gray-60 transition-colors duration-200">{t.nav.login}</a>
          <a href="https://amplitude.com/get-started" target="_blank" rel="noopener noreferrer" className="px-5 py-2.5 bg-amp-blue text-white text-sm font-semibold rounded-md hover:bg-[#0044CC] transition-colors duration-200">{t.nav.startFree}</a>
        </div>
      </div>
    </motion.nav>
  )
