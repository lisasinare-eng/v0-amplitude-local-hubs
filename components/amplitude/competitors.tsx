    ...
    <>
      <div className="bg-amp-gray-10 border-y border-amp-gray-20 py-4 px-6 lg:px-16">
        <div className="max-w-[1200px] mx-auto flex items-center gap-3 flex-wrap">
          ...
          {regionLinks.map((r)=>(<button key={r.id} onClick={()=>setRegion(r.id)} className={`flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-semibold border transition-all duration-200 ${region===r.id?"bg-amp-blue text-white border-amp-blue":"bg-white text-amp-gray-60 border-amp-gray-20 hover:border-amp-blue hover:text-amp-blue"}`}><span>{r.flag}</span><span>{r.label}</span></button>))}
        </div>
      </div>
